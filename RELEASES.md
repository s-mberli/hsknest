...
- **Scheduling research** — retention studies on real review-log data, Leitner+FSRS hybrids, SM-2/FSRS comparison. (FSRS itself already ships as the default algorithm; this is deeper tuning research on top of it.)
- **Word-slicing practice mode** — a fruit-ninja-style arcade round: swipe to slice the correct translation/character out of a set of falling words. Reuses the gesture-first deck and the existing pressure-free practice-mode pattern (never touches the review schedule).
