...
export function SessionPicker({ onHrefChange, onSizeChange }: SessionPickerProps) {
  ...
  return (
    <div className="space-y-2">
      <div className="mx-auto grid max-w-xs grid-cols-2 gap-1 rounded-full bg-muted p-1">
        {(["cards", "minutes"] as const).map((m) => (
          <button
            ...
            onClick={() => setMode(m)}
            className={cn(
              "rounded-full py-1 text-xs font-medium capitalize transition-colors",
              mode === m
      ...
      <div className="mx-auto grid max-w-xs grid-flow-col auto-cols-fr gap-1.5">
        {mode === "cards"
          ? CARD_OPTIONS.map((o) => (
              <button
                key={o.value}
                ...
                className={cn(
                  "rounded-full border py-1 text-xs font-medium transition-colors",
                  cardChoice === o.value
