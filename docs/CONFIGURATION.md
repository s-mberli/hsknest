...
| `READING_MODE_ENABLED` | no  | `true`                        | Ingest Reading Mode's graded stories on container boot (idempotent, same pattern as `AUTO_SEED`). Defaults to `true` — on for everyone, like the rest of the starter content. Set to exactly `"false"` to skip it on a stripped-down instance; existing reading progress and enrolled words are untouched either way, only future re-ingests are skipped. |
