...
| `NEXT_PUBLIC_ENABLE_NINJA` | no | `true` | Enables Word Ninja, a fast-paced slice-practice mode (works for any language — term/phonetic driven, not Chinese-specific). Fails **closed** like `ALLOW_REGISTRATION`: only the exact literal `"true"` turns it on. **Build-time, not runtime** — like the other `NEXT_PUBLIC_*` vars above, it's inlined when the image is built (see `docker-compose.yml`'s `build.args`); changing it and only restarting the container has no effect, you must rebuild. |
