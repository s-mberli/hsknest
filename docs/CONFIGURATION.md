...
| `NEXT_PUBLIC_ENABLE_NINJA` | no | `true` | Enables Word Ninja, a fast-paced slice-practice mode (Chinese only). Fails **closed** like `ALLOW_REGISTRATION`: only the exact literal `"true"` turns it on. Still short of the shipped-mode bar (device matrix, docs) — expect rough edges. |
