...
plus word-level timing marks, not per-term clips. Generated by
`scripts/generate-story-audio.py` (free Microsoft Edge TTS — same engine as
`generate-audio.py` above, no API key — using its WordBoundary events for
timing) and
synced into place by `scripts/sync-audio.py`; output lands under
`public/audio/` alongside a `.json` timings file per story, referenced from
`ReadingAudio` (see `docs/ARCHITECTURE.md`'s Reading Mode section). Story
content itself is authored as markdown and ingested via
